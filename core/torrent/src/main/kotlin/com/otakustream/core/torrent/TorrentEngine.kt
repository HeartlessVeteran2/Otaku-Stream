...
class TorrentEngine @Inject constructor(
    ...
    //
    // Only arm64 is bundled, so on a 32-bit device the class fails to initialize.
    //
    ...
    // libtorrent4j artifact alone. This comment named the one mechanism the build went out of its
    // way not to use, which would send the next reader looking for a setting that is not there. That surfaces as UnsatisfiedLinkError the first time, and then as
