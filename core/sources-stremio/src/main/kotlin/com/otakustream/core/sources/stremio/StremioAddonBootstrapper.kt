...
class StremioAddonBootstrapper @Inject constructor(
    ...
    suspend fun loadPersistedSources(): List<StremioVideoSource> = withContext(Dispatchers.Default) {
        ...
        stremioRepository.getAllAddons().filter { it.enabled }.flatMap { record ->
            runCatching {
                // Re-register any stream/subtitle provider (Torrentio, OpenSubtitles et al.) so
                // catalog-less add-ons keep contributing after a cold start, not only in the
                // session they were installed in.
                installer.registerProviderIfAny(record.manifestUrl, record.manifestJson)
